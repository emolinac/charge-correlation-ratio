...
void macro_print_rc_jet_pt_stringbreakingfrac()
{
    TFile* fin = new TFile((output_folder+namef_rc_jet_pt_data_d).c_str());

    // Get the histograms to create one in top of each other
    TH1F* h_rc_jet_pt_data        = (TH1F*) fin->Get("rc_jet_pt");
    TH1F* h_rc_jet_pt_data_decays = (TH1F*) fin->Get("rc_jet_pt_decays");
