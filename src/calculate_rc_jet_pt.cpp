...
int main()
{
    ...
    for(int i = 0 ; i < N ; i++)
    {
        hcs_samesign[i] = new TH1F(Form("hcs_samesign_%i",i),"",Nbin_jet_pt,jet_pt_limits);
        hcs_diffsign[i] = new TH1F(Form("hcs_diffsign_%i",i),"",Nbin_jet_pt,jet_pt_limits);
        hcs_sum[i]      = new TH1F(Form("hcs_sum_%i",i)     ,"",Nbin_jet_pt,jet_pt_limits);
        hcs_sub[i]      = new TH1F(Form("hcs_sub_%i",i)     ,"",Nbin_jet_pt,jet_pt_limits);
        rc[i]           = new TH1F(Form("rc_%i",i)          ,"",Nbin_jet_pt,jet_pt_limits);
