...
int main()
{
    ...
    TFile* fout = new TFile((output_folder+rc_z_data).c_str(),"RECREATE");
    gROOT->cd();
    ...
    for(int evt = 0 ; evt < datatree->fChain->GetEntries() ; evt++)
    {
        ...
        if(h1_charge*h2_charge<0) diffsign_sigma_z->Fill(datatree->Jet_Dtr_Z[h2_location]);
        else samesign_sigma_z->Fill(datatree->Jet_Dtr_Z[h2_location]);
