...
int main()
{
    ...
    TFile* fout = new TFile((output_folder+namef_rc_z_data).c_str(),"RECREATE");
    gROOT->cd();
    ...
    // ntuple->Project("hcs_diffsign","nlh_z",diffsign_cut_data);
    // ntuple->Project("hcs_samesign","nlh_z",samesign_cut_data);
    ntuple->Project("hcs_diffsign","nlh_pz/(lh_pz+nlh_pz)",diffsign_cut_data);
    ntuple->Project("hcs_samesign","nlh_pz/(lh_pz+nlh_pz)",samesign_cut_data);
